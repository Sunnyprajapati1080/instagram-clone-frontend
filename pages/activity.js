                    ...
                    activities.map((activity, index) => {
                        return (
                            <div key={index} className='pb-1 my-3 ml-1.5 flex relative w-full space-x-2'>
                                <div>
                                    <Link href={`/users/${activity.id}`}><div><Image src={activity.profileImg === "" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + activity.profileImg} className="min-w-[31px] min-h-[31px] object-cover rounded-full" width={31} height={31} /></div></Link>
                                </div>
                                ...
                                {activity.postImg && <Link href={"/posts#" + activity.postId}><div className='absolute right-1.5'><Image src={activity.postImg === "" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + activity.postImg} width={31} height={31} className=" object-cover" /></div></Link>}
                            </div>
                        )
                    })}
            </div>
...
export async function getServerSideProps(context) {
    ...
    const activitiesjson = await fetch(`${process.env.HOST}/api/auth/getactivities`, {
        headers: {
            "token": context.req.cookies.token
        }
    })
    ...
    return {
        props: { profileImg: `${process.env.HOST}/` + resjson.profileImg, activities: resjson2.activities?resjson2.activities:[]}
    }
}
