...
const getUser = (props) => {
    ...
    return (
        <>
            ...
            <Navbar classes="hidden sm:block" img={props.profileImg === "/profile.png" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + props.profileImg} />
            <div className="max-w-[400px] mx-auto py-3 grid place-items-center mt-[50px]">
                <Image width={96} height={96} src={user.profileImg === "/profile.png" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + user.profileImg} className="rounded-full object-cover object-center" />
            ...
            {posts.length === 0 ? <div className='flex items-center max-w-[500px] mx-auto'><a href="http://www.freepik.com"><Image src="/empty.jpg" width={500} height={500} alt="no posts!" /></a></div> : <div className='max-w-[900px] p-1 mx-auto grid grid-cols-3 gap-1'>
            ...
            <BottomNav img={props.profileImg === "/profile.png" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + props.profileImg} />
        </>
    )
}

export async function getServerSideProps(context) {
    const imgjson = await fetch(`${process.env.HOST}/api/auth/getProfileImg`, {
        headers: {
    ...
    const userjson = await fetch(`${process.env.HOST}/api/auth/getuser/${context.query.id}`, {
        headers: {
            "token": context.req.cookies.token
        }
    })
    ...
    const postsjson = await fetch(`${process.env.HOST}/api/posts/getUserPosts/${context.query.id}`, {
        headers: {
            "token": context.req.cookies.token
        }
    })
