            ...
            {following.map((user, index) => {
              return (
                <div key={index} className="relative my-6">
                  <Link href={"/users/" + user._id}><div className='absolute cursor-pointer'><Image src={`${user.profileImg === "" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + user.profileImg}`} width={32} height={32} className="object-cover rounded-full" /></div></Link>
              ...
              {following.map((user, index) => {
                return (
                  <div key={index} className="mb-5">
                    <Link href={"/users/" + user._id}><div className='float-left cursor-pointer'><Image src={`${user.profileImg === "" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + user.profileImg}`} width={32} height={32} className="object-cover rounded-full" /></div></Link>
...
export async function getServerSideProps(context) {
  ...
  const response = await fetch(`${process.env.HOST}/api/auth/getfollowing/${context.query.id}`, {
    headers: {
      "token": context.req.cookies.token
    }
  })
  ...
  return {
    props: { profileImg: `${process.env.HOST}/` + resjson.profileImg, following: following.following }
  }
}
