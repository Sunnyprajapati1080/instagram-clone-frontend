            ...
            {followers.map((follower, index) => {
              return (
                <div key={index} className="relative my-5">
                  <Link href={"/users/" + follower._id}><div className='absolute cursor-pointer'><Image src={`${follower.profileImg === "" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + follower.profileImg}`} width={32} height={32} className="object-cover rounded-full" /></div></Link>
              ...
              {followers.map((follower, index) => {
                return (
                  <div key={index} className="mb-5">
                    <Link href={"/users/" + follower._id}><div className='float-left cursor-pointer'><Image src={`${follower.profileImg === "" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + follower.profileImg}`} width={32} height={32} className="object-cover rounded-full" /></div></Link>
...
export async function getServerSideProps(context) {
  ...
  const response = await fetch(`${process.env.HOST}/api/auth/getfollowers/${context.query.id}`, {
    headers: {
      "token": context.req.cookies.token
    }
  })
  ...
  return {
    props: { profileImg: `${process.env.HOST}/` + resjson.profileImg,followers:followers.followers }
  }
}
