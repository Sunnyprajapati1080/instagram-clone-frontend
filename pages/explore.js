        ...
        <>
            ...
            {posts.length > 0 ? <div className='max-w-[1000px] mt-[50px] sm:mt-[60px] grid grid-cols-3 gap-[3px] md:gap-[5px] p-1 mx-auto'>
                {posts.map((post, index) => {
                    return <Image key={index} onClick={()=>handlePush(post._doc._id)} width={500} height={500} className="object-cover cursor-pointer" src={`${process.env.NEXT_PUBLIC_HOST}/${post._doc.img}`} alt="" />
                })}
            </div> :
...
export async function getServerSideProps(context) {
    ...
    const res2 = await fetch(`${process.env.HOST}/api/posts/getAllPosts`, {
        headers: {
            "token": context.req.cookies.token
        }
    })
    ...
    return {
        props: { profileImg: `${process.env.HOST}/` + imgjson.profileImg, posts: postsjson }
    }
}
