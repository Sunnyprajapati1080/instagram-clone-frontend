...
const Profile = (props) => {
    ...
    return (
        <>
            ...
            <Navbar classes="hidden sm:block" img={props.userDetails.profileImg === "/profile.png" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + props.userDetails.profileImg} />
            <div className="max-w-[400px] mx-auto py-3 grid place-items-center mt-[50px]">
                <Image width={96} height={96} loading="lazy" src={props.userDetails.profileImg === "/profile.png" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + props.userDetails.profileImg} className="rounded-full object-cover object-center" />
            ...
            <BottomNav img={props.userDetails.profileImg === "/profile.png" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + props.userDetails.profileImg} />
        </>
    )
}

export async function getServerSideProps(context) {
    const res1 = await fetch(`${process.env.HOST}/api/auth/getUserDetails`, {
        headers: {
    ...
    const res2 = await fetch(`${process.env.HOST}/api/posts/getOwnPosts`, {
        headers: {
            "token": context.req.cookies.token
        }
    })
