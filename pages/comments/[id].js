                        ...
                        {comments.map((comment, index) => {
                            return (
                                <div key={index} className="relative my-5">
                                    <Link href={"/users/" + comment.user._id}><div className='absolute cursor-pointer'><Image src={`${comment.user.profileImg === "" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + comment.user.profileImg}`} width={30} height={30} className="object-cover rounded-full" /></div></Link>
                            ...
                            comments.map((comment, index) => {
                                return (
                                    <div key={index} className="mb-5">
                                        <Link href={"/users/" + comment.user._id}><div className='float-left cursor-pointer'><Image src={`${comment.user.profileImg === "" ? "/profile.png" : process.env.NEXT_PUBLIC_HOST + "/" + comment.user.profileImg}`} width={30} height={30} className="object-cover rounded-full" /></div></Link>
...
export async function getServerSideProps(context) {
    const res = await fetch(`${process.env.HOST}/api/auth/getProfileImg`, {
        headers: {
    ...
    const response = await fetch(`${process.env.HOST}/api/posts/getcomments/${context.query.id}`, {
        headers: {
            "token": context.req.cookies.token
        }
    })
    ...
    return {
        props: { img: resjson.profileImg, profileImg: `${process.env.HOST}/` + resjson.profileImg, comments: post.comments, postId: post._id }
    }
}
