...
const SignUp = () => {
    ...
    const handleOnFileUpload = (e) => {
        if (e.target.files[0].type === "image/jpeg" || e.target.files[0].type === "image/webp" || e.target.files[0].type === "image/png") {
            setfileName(e.target.files[0].name)
            return seterrors({ ...errors, fileError: false })
        } else return seterrors({ ...errors, fileError: true })
    }
    return (
                    ...
                    <div className="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                        <div className="w-full">
                            ...
                            <form encType="multipart/form-data" onSubmit={usercontext.handleOnSignUp} className="relative">
                                ...
                                <input onChange={handleOnFileUpload} type="file" id="profileImg" name="profileImg" className="hidden absolute mt-1" />
                                {<p className={`text-rose-500 text-sm ${errors.fileError? "block" : "hidden"}`}>only jpg, png and webp formats are supported!</p>}
                                <button
